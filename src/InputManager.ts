    ...
    window.addEventListener("mousemove", (e) => {
      var movementX = e.movementX ||
        ...
        movementY = e.movementY ||
          0;
      if (movementX != null) {
        this.listeners["pointermove"]({ mx: movementX / window.innerWidth, my: -movementY / window.innerHeight });
      } else {
        ...
        if (prevMousePos.x != 0) {
          this.listeners["pointermove"]({ mx: this.mousePos.x - prevMousePos.x, my: this.mousePos.y - prevMousePos.y });
        }
      }
    }, false);
